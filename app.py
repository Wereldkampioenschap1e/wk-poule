...
HUIDIGE_RONDE = "Ronde 3"   # ← HIER AANPASSEN per ronde
...
# Regex: geldige uitslag – bijv. 0-0 t/m 10-10
    ...
    if st.session_state.get(f"ingestuurd_{HUIDIGE_RONDE}", False):
        ...
        with st.form(f"wk_poule_{HUIDIGE_RONDE.lower().replace(' ', '_')}", border=False):
            ...
            with st.container(border=True):
                nickname_val = st.text_input(
                    "Nickname *",
                    placeholder="Bijv. VoetbalFan88",
                    key="nickname",
                )
            ...
            for tab_idx, (wtab, start) in enumerate(
                zip(wed_tabs, [0, 6, 12, 18])
            ):
                with wtab:
                        ...
                        with st.expander(f"⚽ {wed}"):
                            ...
                            if wed == "Nederland - Japan":
                                st.markdown(
                                    ...
                                    "jij het eerste schot op doel van Nederland?"
                                )
                                oranje_min_val = st.number_input(
                                    "Minuut 1e schot op doel",
                                    min_value=1,
                                    max_value=90,
                                    value=15,
            ...
            topscorer_val = st.multiselect(
                ...
                key="topscorers",
                help=(
                    "Selecteer exact 4 spelers waarvan jij denkt dat ze "
                    "topscorer worden. Alle 48 WK-selecties zijn beschikbaar."
                ),
            )
            if topscorer_val:
        ...
        if submitted:
            ...
            if len(topscorer_val) != 4:
                fouten.append(
                    ...
                    f"(nu {len(topscorer_val)} geselecteerd)."
                )
            ...
            if uitslag_fouten:
                fouten.append(
                    "Ongeldige of ontbrekende uitslag bij: "
                    + ", ".join(uitslag_fouten) + "."
                )
            ...
            if fouten:
                ...
                try:
                    ...
                    pass  # bij verbindingsfout: toch doorgaan
                ...
                if can_submit:
                    ...
                    try:
                        ...
                        if resp_post.status_code == 200:
                            ...
                            st.error(
                                f"Er is een fout opgetreden bij het opslaan "
